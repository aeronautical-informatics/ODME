...
public class MermaidExporter {
    ...
    public static String export(Node root) {
        ...
        int[] counter = {0};
        collectLines(root, counter, nodeLines, edgeLines, -1, new int[0]);
    ...
    private static int collectLines(Node node, int[] counter, List<String> nodeLines,
                                    List<String> edgeLines, int parentId, int[] unused) {
        int myId = counter[0]++;
        ...
        for (Node child : node.getChildren()) {
            collectLines(child, counter, nodeLines, edgeLines, myId, new int[0]);
        }
        return myId;
    }
}
